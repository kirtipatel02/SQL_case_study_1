...
select store_id, count(*) active_custormers 
   from customer
   where active = 1
   group by store_id;
...
*/
select count(email) as count_of_customers_email
   from customer;
...
*/
select store_id, count(distinct title) as count_of_unique_title, count(distinct category.name) as unique_categories
   from category,inventory as i
   left join film as f
